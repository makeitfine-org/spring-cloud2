package reacty.probe.one.inventory.order.config;

import reacty.probe.one.inventory.order.event.DeliveryScheduledEvent;
import reacty.probe.one.inventory.order.event.InventoryFailedEvent;
import reacty.probe.one.inventory.order.event.InventoryReservedEvent;
...
public class KafkaConfig {
    ...
    @Bean
    public ProducerFactory<String, OrderCreatedEvent> orderProducerFactory(ObjectMapper objectMapper) {
    ...
    public ConsumerFactory<String, InventoryFailedEvent> inventoryFailedConsumerFactory() {
        ...
        props.put(ErrorHandlingDeserializer.VALUE_DESERIALIZER_CLASS, JsonDeserializer.class);
        props.put(JsonDeserializer.TRUSTED_PACKAGES, "*");
        props.put(JsonDeserializer.VALUE_DEFAULT_TYPE, InventoryFailedEvent.class.getName());
        props.put(JsonDeserializer.USE_TYPE_INFO_HEADERS, false);
        return new DefaultKafkaConsumerFactory<>(props);
    }
    ...
    @Bean
    public ConsumerFactory<String, DeliveryScheduledEvent> deliveryScheduledConsumerFactory() {
        Map<String, Object> props = new HashMap<>();
        props.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServers);
        props.put(ConsumerConfig.GROUP_ID_CONFIG, "order-service-group");
        props.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        ...
        props.put(ErrorHandlingDeserializer.VALUE_DESERIALIZER_CLASS, JsonDeserializer.class);
        props.put(JsonDeserializer.TRUSTED_PACKAGES, "*");
        props.put(JsonDeserializer.VALUE_DEFAULT_TYPE, DeliveryScheduledEvent.class.getName());
        props.put(JsonDeserializer.USE_TYPE_INFO_HEADERS, false);
        return new DefaultKafkaConsumerFactory<>(props);
    }

    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, DeliveryScheduledEvent> deliveryScheduledListenerFactory(
            ConsumerFactory<String, DeliveryScheduledEvent> consumerFactory) {
        ConcurrentKafkaListenerContainerFactory<String, DeliveryScheduledEvent> factory = new ConcurrentKafkaListenerContainerFactory<>();
        factory.setConsumerFactory(consumerFactory);
        return factory;
    }
}
